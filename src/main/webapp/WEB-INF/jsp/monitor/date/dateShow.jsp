...
function check(){
	if($("#date").val() == 1){
		window.location.href="<%=request.getContextPath()%>/monitor/dateShowExport/1";
		return true;
	}else{
		alert("没有数据，不能导出！");
		return false;
           ...
           <table width="100%" height="23" border="0" cellpadding="0" cellspacing="5" class="right_set1">
				<tr id="customertr" class=VwCtr style="display:">
					<td width="100%">
		              ...
		              <input type ="button" id="btnval" value="返回" class="input_button2" onclick="javascript:location.href='<%=request.getContextPath()%>/monitor/back/1'"/>
					</td>
				</tr>
				<tr>
					<td colspan="4">

...
<%@page import="cn.explink.util.StringUtil"%>
 ...
 int type = Integer.parseInt(request.getAttribute("flowType").toString());
 %> 
...
<div class="right_box">
	...
	<tr>
		<td align="left">
		<input type ="button" id="back" value="返回" class="input_button2" onclick="javascript:location.href='<%=request.getContextPath()%>/monitordelivery/back'"/>
	...
	<form action="<%=request.getContextPath()%>/monitordelivery/dateShowExport" method="post" id="searchForm2">
	...
	<table width="5000" border="0" cellspacing="1" cellpadding="0" class="table_2" id="gd_table">
	   <tr class="font_1">
				...
				<td  align="center" valign="middle" bgcolor="#eef6ff" onclick="orderForm('newfollownotes');" >订单当前状态</td>
		...
		<% for(CwbOrder c : showDateList){ %>
				<tr bgcolor="#FF3300">
					...
					<td  align="center" valign="middle"><%if(type==6 ){%><strong><%=c.getOutstoreroomtime() %></strong><%}else{ %><%=c.getOutstoreroomtime() %><%} %></td>
					<td  align="center" valign="middle"><%if(type==7 ){%><strong><%=c.getInSitetime() %></strong><%}else{ %><%=c.getInSitetime() %><%} %></td>
					<td  align="center" valign="middle"><%if(type==9 ){%><strong><%=c.getPickGoodstime() %></strong><%}else{ %><%=c.getPickGoodstime() %><%} %></td>

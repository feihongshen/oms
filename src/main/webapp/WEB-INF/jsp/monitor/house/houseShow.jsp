...
<%@page import="cn.explink.util.StringUtil"%>
 <%List<CwbOrder> showDateList =(List<CwbOrder>) request.getAttribute("cwborderList"); 
 int usershowphoneflag = Integer.parseInt(request.getAttribute("usershowphoneflag").toString());
 List<Exportmould> exportmouldlist = (List<Exportmould>)request.getAttribute("exportmouldlist");
 String datetype = request.getAttribute("datetype").toString();
 long carwarehouse = Long.parseLong(request.getAttribute("carwarehouse").toString());
 %> 
...
<div class="right_box">
	...
	<tr>
		<td align="left">
		<input type ="button" id="back" value="返回" class="input_button2" onclick="javascript:location.href='<%=request.getContextPath()%>/monitorhouse/date'"/>
		...
		<%-- <font color="red" style="font-size:12px">&nbsp;&nbsp;<%=request.getAttribute("flowTypeStr")==null?"":request.getAttribute("flowTypeStr")%></font> --%>
		</td>
	</tr>
		...
		<table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_1">
			<tr>
				<td height="38" align="center" valign="middle" bgcolor="#eef6ff">
					<a href="javascript:$('#searchForm').attr('action','1?datetype=<%=datetype %>&carwarehouse=<%=carwarehouse %>');$('#searchForm').submit();" >第一页</a>　
					<a href="javascript:$('#searchForm').attr('action','<%=page_obj.getPrevious()<1?1:page_obj.getPrevious() %>?datetype=<%=datetype %>&carwarehouse=<%=carwarehouse %>');$('#searchForm').submit();">上一页</a>　
					<a href="javascript:$('#searchForm').attr('action','<%=page_obj.getNext()<1?1:page_obj.getNext() %>?datetype=<%=datetype %>&carwarehouse=<%=carwarehouse %>');$('#searchForm').submit();" >下一页</a>　
					<a href="javascript:$('#searchForm').attr('action','<%=page_obj.getMaxpage()<1?1:page_obj.getMaxpage() %>?datetype=<%=datetype %>&carwarehouse=<%=carwarehouse %>');$('#searchForm').submit();" >最后一页</a>
					　共<%=page_obj.getMaxpage() %>页　共<%=page_obj.getTotal() %>条记录 　当前第<select
							id="selectPg"
							onchange="$('#searchForm').attr('action',$(this).val()?datetype=<%=datetype %>&carwarehouse=<%=carwarehouse %>);$('#searchForm').submit()">

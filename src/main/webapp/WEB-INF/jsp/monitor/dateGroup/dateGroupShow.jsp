...
function check(){
	if($("#date").val() == 1){
		window.location.href="<%=request.getContextPath()%>/monitor/dateShowExport/2";
		return true;
	}else{
		alert("没有数据，不能导出！");
		return false;
		...
		<form name="form1" id="searchForm" action ="<%=request.getContextPath()%>/order/select/1" method = "post">					
           <table width="100%" height="23" border="0" cellpadding="0" cellspacing="5" class="right_set1">
				<tr id="customertr" class=VwCtr style="display:">
					<td width="100%">
					  <form method ="post" action ="1" id ="searchForm"></form>
					  <input type ="button" id="btnval" value="导出excel" class="input_button2" onclick="check();"/>
		              <input type ="button" id="btnval" value="返回" class="input_button2" onclick="javascript:location.href='<%=request.getContextPath()%>/monitor/back/2'"/>
					</td>
				</tr>
				<tr>
					<td colspan="4">
					   <table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_2" id="gd_table2">
	                     ...
	                     <% for(int i=((page1-1)*10);i<showDateList.size();i++){ %>
	                      <% MonitorOrderDTO mo = showDateList.get(i); %> 
	                       <%if(i< page1*10 ){ %>
							<tr>
								<td><%=mo.getCwb() %></td>

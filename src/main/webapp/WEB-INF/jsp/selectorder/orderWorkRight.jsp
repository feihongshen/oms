...
Switch sw = (Switch)request.getAttribute("sw");
long userRoleid = Long.parseLong(request.getAttribute("userRoleid").toString());
%>
<HEAD>
...
function updateMack(cwb){
	var type = "0";
	if($("#quxiaoMackButton").html().length<3){
		type ="1";
	}
	$.ajax({
		type: "POST",
		...
		success : function(data) {
			if(data.errorCode==0){
				alert(data.error);
				if($("#quxiaoMackButton").html().length<3){
					$("#quxiaoMackButton").html("取消标记");
				}else if($("#quxiaoMackButton").html().length>3){
					$("#quxiaoMackButton").html("标记");
				}
			}else{
				alert(data.error);
			}
		...
		<tbody>
			...
			<%if(oList!=null && oList.size()>0){ %>
				<td valign="top"  width="500">
					<form action="<%=request.getContextPath()%>/order/selectOrder/1" id="seachForm" method="post">
						<table width="100%" border="0" cellspacing="1" cellpadding="2">
							<tbody>
								...
								<tr>
									<td width="10%" align="left" valign="middle" style="font-size:12px">&nbsp;&nbsp;单号：
										<input type="text" name="cwb" id="orderSearch" value="">
										按回车键查询
									</td>
								</tr>
							</tbody>
							...
							<tbody>
								<tr class="font_1">
									...
									<span style="float:right">
									  <%if(o.getMarksflag() == 1){ %>
									    <a  id="quxiaoMackButton" onclick="updateMack('<%=o.getCwb()%>')" class="button_a">取消标记</a>
									  <%} else{%> 
									    <a  id="quxiaoMackButton" onclick="updateMack('<%=o.getCwb()%>')" class="button_a">标记</a>
									  <%} %>
									  <a  class="button_a" onclick="dgetViewBox('<%=o.getCwb()%>')" >操作日志</a>
									</span>
									  <h1>&nbsp;订单详情</h1>
									</td>
								...
								<tr>
									<td width="10%" align="left" colspan="2" valign="middle">
										<table width="100%" border="0" cellspacing="0" cellpadding="0" class="right_set1" style="height:127px">
											<tbody>
												<tr>
												...
												<tr>
													<td><b>签收时间：</b><input name="flagtime" type="text" id="flagtime" value="<%=o.getEditsignintime()==null||o.getEditsignintime().length()==0?"当前时间":o.getEditsignintime()%>" /><%if(sw.getState().length()==0 || !((sw.getState().substring(sw.getState().length()-1).equals("1"))) || userRoleid==1) {%><input type="button" class="input_button2"  id="bqianshou" value="修改" /><%} %></td>
													<td><b>签收人：</b><input name="flagname" type="text" id="flagname" value="<%=o.getSigninman()==null?"":o.getSigninman()%>" /><%if(sw.getState().length()==0 || !((sw.getState().substring(sw.getState().length()-1).equals("1"))) || userRoleid==1) {%><input type="button" class="input_button2"  id="bqianshouren" value="修改" /><%} %></td>
												</tr>	
												<tr>
					...
					<form action="<%=request.getContextPath()%>/order/update" id="payChangeForm" method="post">
					<input type="hidden" value="<%=request.getParameter("cwb")==null?"":request.getParameter("cwb")%>" name="cwb">
								...
								<td>
									<table width="100%" border="0" cellspacing="1" cellpadding="2" class="table_2">
										<tbody>
											<tr class="font_1">
												<td width="10%" height="26" align="left" valign="middle" bgcolor="#00AEF0">
												 <span style="float:right"><a id="payChange" class="button_a">保存日志</a></span>
												 <h1>&nbsp;订单备注</h1>
												</td>
											</tr>
											<tr>
												<td width="10%" align="left" valign="middle" class="right_set1" style="height:40px"><span style="height:20px">
												<textarea name="cwbremack" rows="2" id="cwbremackID" style="width:85%;height:60px"></textarea>
												</span>
												</td>
											</tr>
										</tbody>
								...
								<td>
									<table width="100%" border="0" cellspacing="1" cellpadding="2" class="table_2">
										<tbody>
											...
											<tr>
												<td width="10%" align="left" valign="middle" class="right_set1" style="height:40px"><span style="height:20px">
												<textarea name="cwbremack" rows="2" id="textfield" style="width:85%;height:60px"></textarea>
												</span>
												</td>
											</tr>
										</tbody>

...
<script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#orderResultType").multiSelect({ oneOrMoreSelected: '*' });
   //获取下拉框的值
   $("#find").click(function(){
	  	 $("#isshow").val("1");
        var checkval="";
...
$(document).ready(function() {
	...
	$("#find2").click(function(){
		if(check()){
			if($.trim($("#cwbandtranscwbId").val()).length==0){
				alert("请输入订单号");
				return false;
			}
		 	$("#isshow").val(1);
...
function updateStartBranchDeliver(){
	$.ajax({
		url:"<%=request.getContextPath()%>/advancedquery/updateDeliver",//后台处理程序
		type:"POST",//数据发送方式 
		data:"branchid="+$("#startbranchid").val(),//参数
		dataType:'json',//接受数据格式
		success:function(json){
			$("#startdelivername").empty();//清空下拉框//$("#select").html('');
			$("<option value='-1'>请选择小件员</option>").appendTo("#startdelivername");//添加下拉框的option
			for(var j = 0 ; j < json.length ; j++){
				$("<option value='"+json[j].userid+"'>"+json[j].realname+"</option>").appendTo("#startdelivername");
				}
			}		
	});
}
function updateNextBranchDeliver(){
	$.ajax({
		url:"<%=request.getContextPath()%>/advancedquery/updateDeliver",//后台处理程序
		type:"POST",//数据发送方式 
		data:"branchid="+$("#nextbranchid").val(),//参数
		dataType:'json',//接受数据格式
		success:function(json){
			$("#nextdelivername").empty();//清空下拉框//$("#select").html('');
			$("<option value='-1'>请选择小件员</option>").appendTo("#nextdelivername");//添加下拉框的option
			for(var j = 0 ; j < json.length ; j++){
				$("<option value='"+json[j].userid+"'>"+json[j].realname+"</option>").appendTo("#nextdelivername");
				}
			}		
	});
}
...
function clearSelect(){
	...
	$("#carsize").val('');//体积
	$("#startdelivername").empty();//清空下拉框//$("#select").html('');
	$("#nextdelivername").empty();
	$("<option value='-1'>请选择当前小件员</option>").appendTo("#startdelivername");//添加下拉框的option
	$("<option value='-1'>请选择配送小件员</option>").appendTo("#nextdelivername");//添加下拉框的option
	$("#customerwarehouseid").empty();//清空下拉框//$("#select").html('');
	$("<option value='-1'>请选择发货仓库</option>").appendTo("#customerwarehouseid");//添加下拉框的option
}
</script>
...
<div class="right_box">
	...
	<tr>
		<td width="60" rowspan="4" align="left" valign="top">订单号：
			</td>
		<td width="160" rowspan="4" align="left" valign="top">
			<textarea cols="24" rows="5"  name ="cwbandtranscwb" id="cwbandtranscwbId"></textarea>
			</td>
		<td width="30" rowspan="4" align="left" valign="top">
		<br/><br/><br/><br/><br/>
			<input type="button" id="find2"  value="查询" class="input_button2" />
		</td>
		<td align="left">
		   ...
		   <select name ="customerwarehouseid" id ="customerwarehouseid">
		          <option value ="-1">请选择发货仓库</option>
		           <%if(customWareHouseList!=null && customWareHouseList.size()>0) {%>
			...
			<select name ="startbranchid" id ="startbranchid" onchange="updateStartBranchDeliver();">
		          <option value ="-1">请选择当前站点</option>
			...
			<select id="startdelivername" name ="startdelivername">
		    <option value ="-1">请选择当前站点小件员</option>
		        <%if(startBranchUserlist!=null && startBranchUserlist.size()>0) {%>
		          <%for(User c : startBranchUserlist){ %>
			 <option value =<%=c.getUserid() %> 
		           <%if(c.getUserid()==Integer.parseInt(request.getParameter("startdelivername")==null?"-1":request.getParameter("startdelivername"))){ %>selected="selected" <%} %> ><%=c.getRealname()%></option>
		          <%} }%>
			</select>
			
			<select name ="nextbranchid" id ="nextbranchid" onchange="updateNextBranchDeliver();">
		          <option value ="-1">请选择配送站点</option>
		          <%if(branchlist!=null && branchlist.size()>0){ %>
		          <%for(Branch c : branchlist){ %>
		          <option value =<%=c.getBranchid() %> 
		           <%if(c.getBranchid()==Integer.parseInt(request.getParameter("nextbranchid")==null?"-1":request.getParameter("nextbranchid"))){ %>selected="selected" <%} %> ><%=c.getBranchname()%></option>
		          <%} }%>
			 </select>
			 
			<select id="nextdelivername" name ="nextdelivername">
		    <option value ="-1">请选择配送小件员</option>
		        <%if(nextBranchUserlist!=null && nextBranchUserlist.size()>0) {%>
		          <%for(User c : nextBranchUserlist){ %>
			 <option value =<%=c.getUserid() %> 
		           <%if(c.getUserid()==Integer.parseInt(request.getParameter("nextdelivername")==null?"-1":request.getParameter("nextdelivername"))){ %>selected="selected" <%} %> ><%=c.getRealname()%></option>
		          <%} }%>
			</select>
			货物重量(kg)：<input type="text" name="beginWatht" id="beginWatht"
			onkeyup="value=value.replace('０','0').replace('１','1').replace('２','2').replace('３', '3').replace('４','4').replace('５','5').replace('６','6').replace('７', '7').replace('８','8').replace('９','9');checkBeishu('beginWatht',999999);" 
			value="<%=request.getParameter("beginWatht")==null ||Integer.parseInt(request.getParameter("beginWatht").equals("")?"-1":request.getParameter("beginWatht"))==-1 ?"":request.getParameter("beginWatht") %>" size="8"/>到
			<input type="text" name="endWatht" id="endWatht"
			onkeyup="value=value.replace('０','0').replace('１','1').replace('２','2').replace('３', '3').replace('４','4').replace('５','5').replace('６','6').replace('７', '7').replace('８','8').replace('９','9');checkBeishu('endWatht',999999);"
			value="<%=request.getParameter("endWatht")==null ||Integer.parseInt(request.getParameter("endWatht").equals("")?"-1":request.getParameter("endWatht"))==-1 ?"":request.getParameter("endWatht") %>" size="8"/>
			</td>
	</tr>
	<tr>
		<td align="left">
		...
		<select id="datetype" name ="datetype">
            ...
            <option value="6">配送成功时间</option>
            <option value="7">反馈时间</option>
            <option value="8">归班时间</option>
            <option value="9">反馈滞留时间</option>
		</select>
	...
	订单配送结果:
		<select name =orderResultType id ="orderResultType" multiple="multiple" style="width: 320px;">
               		<%for(OrderResultTypeEnum c : OrderResultTypeEnum.values()){ %>
		          <option value =<%=c.getValue() %> 
		...
		<input type="hidden" id="controlStr" name="orderResultTypeStr" value=""/>
	...
	<table width="5000" border="0" cellspacing="1" cellpadding="0" class="table_2" id="gd_table">
	   <tr class="font_1">
				...
				<td  align="center" valign="middle" bgcolor="#eef6ff" onclick="orderForm('editsignintime');" >签收时间</td>
				...
				<td  align="center" valign="middle" bgcolor="#eef6ff" onclick="orderForm('startbranchname');" >上一站</td>
				<td  align="center" valign="middle" bgcolor="#eef6ff" onclick="orderForm('currentbranchname');" >当前站</td>
				...
				<td  align="center" valign="middle" bgcolor="#eef6ff" onclick="orderForm('newfollownotes');" >当前订单状态</td>
				...
				<td  align="center" valign="middle" bgcolor="#eef6ff" onclick="orderForm('backreason');" >退货原因</td>
				<!-- <td  align="center" valign="middle" bgcolor="#eef6ff" onclick="orderForm('leavedreasonid');" >滞留原因id</td> -->
				<td  align="center" valign="middle" bgcolor="#eef6ff" onclick="orderForm('leavedreasonStr');" >滞留原因</td>
				<td  align="center" valign="middle" bgcolor="#eef6ff" onclick="orderForm('expt_code');" >供货商异常原因编号</td>
				<td  align="center" valign="middle" bgcolor="#eef6ff" onclick="orderForm('expt_msg');" >供货商异常原因</td>
				<td  align="center" valign="middle" bgcolor="#eef6ff" onclick="orderForm('orderResultType');" >配送结果</td>
				<td  align="center" valign="middle" bgcolor="#eef6ff" onclick="orderForm('podremarkStr');" >配送结果备注</td>
		...
		<% for(CwbOrder c : orderlist){ %>
				<tr bgcolor="#FF3300">
					...
					<td  align="center" valign="middle"><%=c.getEditsignintime() %></td>
					...
					<td  align="center" valign="middle"><%=c.getOrderResultTypeText() %></td>
	...
	<tr>
	<td>订单总计：<font color="red"><%=request.getAttribute("count") %></font>&nbsp;单  </td>
	</tr>
	<tr>

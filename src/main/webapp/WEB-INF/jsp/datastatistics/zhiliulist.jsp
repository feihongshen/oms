...
function clearSelect(){
	...
	multiSelectAll('cwbordertypeid',0,'请选择订单类型');
	$("#branchname").val("");
...
<div class="right_box">
	...
	<tr>
		<td align="left">
			...
			<select name ="customerid" id ="customerid" multiple="multiple" style="width: 300px;">
		        ...
		        <br/>
		        站点名称：<select name="ismohu" id="ismohu" >
			...
			<select name ="dispatchbranchid" id ="dispatchbranchid"  multiple="multiple" style="width: 320px;">
		          ...
		          <option value ="<%=b.getBranchid() %>"<%if(!dispatchbranchidList.isEmpty()) 
			...
			<select name ="cwbordertypeid" id ="cwbordertypeid" multiple="multiple" >
		          <%for(CwbOrderTypeIdEnum c : CwbOrderTypeIdEnum.values()){ %>
						<option value ="<%=c.getValue() %>"  <%if(!cwbordertypeidList.isEmpty()) 
	...
	<form action="<%=request.getContextPath()%>/datastatistics/exportExcle" method="post" id="searchForm2">
		...
		<input type="hidden" name="sign" id="sign" value="<%=ModelEnum.ZhiLiuDingdanhuizong.getValue() %>"/>
	...
	<table width="1500" border="0" cellspacing="1" cellpadding="0" class="table_2" id="gd_table">
	   <tr class="font_1">
				...
				<td  align="center" valign="middle" bgcolor="#eef6ff" onclick="orderForm('cwb');" >再次配送时间</td>
		...
		<% for(CwbOrder  c : orderlist){ %>
				<tr bgcolor="#FF3300">
					...
					<td  align="center" valign="middle"><%=c.getResendtime() %></td>

...
<div class="right_box">
	...
	<tr>
		<td align="left">
			...
			<select name ="customerid" id ="customerid" multiple="multiple" style="width: 300px;">
		     ...
		     <br/>
			 退货站是否入库：
			<select name ="istuihuozhanruku" id ="istuihuozhanruku">
				  <option value="0" <%if(0==(request.getParameter("istuihuozhanruku")==null?0:Long.parseLong(request.getParameter("istuihuozhanruku")))){ %>selected="selected" <%} %> >全部</option>
		          <option value ="1" <%if(1==(request.getParameter("istuihuozhanruku")==null?0:Long.parseLong(request.getParameter("istuihuozhanruku")))){ %>selected="selected" <%} %> >是</option>
	...
	<form action="<%=request.getContextPath()%>/datastatistics/exportExcle" method="post" id="searchForm2">
		<input type="hidden" name="sign" id="sign" value="<%=ModelEnum.TuiHuoChuZhanTongJi.getValue() %>"/>
		...
		<input type="hidden" name="istuihuozhanruku1" id="istuihuozhanruku1" value="<%=request.getParameter("istuihuozhanruku")==null?"0":request.getParameter("istuihuozhanruku") %>"/>

...
function reIframeTopAndBottomXY(){
	...
	if($("#scroll")){
		$("#scroll").css("height", st+ch-190);
	}
	if($("#scroll2")){
		$("#scroll2").css("height", st+ch-80);
	}
	if($(".form_btnbox")){
		$(".form_btnbox").css("top", st+ch-40);
	}
	if($(".inputselect_box")){
		$(".inputselect_box").css("top", st);
	}
	if($(".list_topbar")){
		$(".list_topbar").css("top", st+35);
	}
	if($(".list_topbar2")){
		$(".list_topbar2").css("top", st+89);
	}
	if($(".paging")){
		$(".paging").css("top", st+460);
	}
...
function updateMack(cwb){
	var type = "0";
	if($("#quxiaoMackButton_"+cwb).html()=="标记"){
		type ="1";
	}
	$.ajax({
		type: "POST",
		url:$("#updateMack_url").val(),
		...
		success : function(data) {
			if(data.errorCode==0){
				alert(data.error);
				if($("#quxiaoMackButton_"+cwb).html()=="标记"){
					$("#quxiaoMackButton_"+cwb).html("取消标记");
				}else{
					$("#quxiaoMackButton_"+cwb).html("标记");
				}
			}else{
				alert(data.error);
			}

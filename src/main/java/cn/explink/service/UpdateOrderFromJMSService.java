...
public class UpdateOrderFromJMSService {
	...
	public void updatebatchedit(@Header("emaildate") String parm, @Header("MessageHeaderUUID") String messageHeaderUUID) {
		...
		try {
			...
			// 把未完成MQ插入到数据库中, start			
			//消费MQ异常表
			this.mqExceptionDAO.save(MqExceptionBuilder.getInstance().buildExceptionCode(this.getClass().getSimpleName() + ".updatebatchedit")
	...
	public void updateLoseOrder(@Header("loseCwbByEmaildateid") String parm, @Header("MessageHeaderUUID") String messageHeaderUUID) {
		...
		try {
			...
			// 把未完成MQ插入到数据库中, start
			//消费MQ异常表
			this.mqExceptionDAO.save(MqExceptionBuilder.getInstance().buildExceptionCode(this.getClass().getSimpleName() + ".updateLoseOrder")

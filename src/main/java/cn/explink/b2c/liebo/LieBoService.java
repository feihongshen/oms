			...
			if (list1.get(0).getTranscwb() != null && !"".equals(list1.get(0).getTranscwb())) {
				...
				if (status == 200) {
					...
					if (jsonArray.size() > 10) {// 退货完成
						cwbDAO.updateByTranscwb(DeliveryStateEnum.QuanBuTuiHuo.getValue(), signintime, list1.get(0).getCwb(), false);
						cwbDAO.saveCwbByTranscwb(allfollownotes, newfollownotes, signintime, list1.get(0).getTranscwb());
					} else if (state == 3) {// 做签收处理
						cwbDAO.updateByTranscwb(DeliveryStateEnum.PeiSongChengGong.getValue(), signintime, list1.get(0).getCwb(), true);
						cwbDAO.saveCwbByTranscwb(allfollownotes, newfollownotes, signintime, list1.get(0).getTranscwb());

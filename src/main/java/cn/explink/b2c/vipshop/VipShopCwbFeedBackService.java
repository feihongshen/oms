...
public class VipShopCwbFeedBackService {
	...
	public long feedback_status(int vipshop_key) {
		...
		try {
			...
			this.sendCwbStatus_To_VipShop_LanTui(vipshop, FlowOrderTypeEnum.YiFanKui.getValue());
	...
	private String AppendXMLString_peisong(VipShop vipshop, List<B2CData> vipshopDataList, int flowordertype, String request_time) {
		...
		for (B2CData b2cData : vipshopDataList) {
			...
			try{
				...
				if(note== null){
					this.logger.info("当前推送唯品会配送单cwb={}的jsoncontent格式有错，无法解析！jsoncontent={}", b2cData.getCwb(), jsoncontent);
					continue;
				}
	
				if (note.getCwbordertypeid() != CwbOrderTypeIdEnum.Peisong.getValue()) {
					this.logger.info("当前推送唯品会cwb={}过滤揽退单,flowordertype={}", b2cData.getCwb(), b2cData.getFlowordertype());
					continue;
				}
	...
	private String AppendXMLString_lantui(VipShop vipshop, List<B2CData> vipshopDataList, int flowordertype, String request_time) {
		...
		for (B2CData b2cData : vipshopDataList) {
			...
			try{
				...
				if(note == null){
					this.logger.info("当前推送唯品会上门退订单cwb={}的jsoncontent格式有错，无法解析！jsoncontent={}", b2cData.getCwb(), jsoncontent);
					continue;
				}				
				...
				logger.error("当前推送唯品会上门退订单[cwb={}]异常,ExceptMsg={}", b2cData.getCwb()==null?"":b2cData.getCwb(), ex.getMessage());
			}
			//Added end
		}
		sub1.append("</traces>");
		...
		private String AppendXMLString_OXO(VipShop vipshop, List<B2CData> vipshopDataList, int flowordertype, String request_time) {
			...
			for (B2CData b2cData : vipshopDataList) {
				...
				try{
					...
					VipShopXMLNote note = this.getVipShopXMLNoteMethod(jsoncontent);
					if(note== null){
						this.logger.info("当前推送唯品会OXO订单cwb={}的jsoncontent格式有错，无法解析！jsoncontent={}", b2cData.getCwb(), jsoncontent);
						continue;
					}
	
					if (note.getCwbordertypeid() != CwbOrderTypeIdEnum.OXO.getValue()) {
						this.logger.info("当前推送唯品会cwb={}过滤非OXO订单,flowordertype={}", b2cData.getCwb(), b2cData.getFlowordertype());
						continue;
					}
					...
					logger.error("当前推送唯品会OXO订单[cwb={}]异常,ExceptMsg={}", b2cData.getCwb()==null?"":b2cData.getCwb(), ex.getMessage());
				}
				//Added end
			}
			sub1.append("</traces>");
	...
	public String getVipShopFlowEnum(long flowordertype, long delivery_state, long cwbordertypeid) {
		...
		if (cwbordertypeid == CwbOrderTypeIdEnum.Peisong.getValue()||cwbordertypeid == CwbOrderTypeIdEnum.OXO.getValue()) {
			...
			if (flowordertype == FlowOrderTypeEnum.YiFanKui.getValue()) {//反馈推送tms 2016-06-22
				if (delivery_state == DeliveryStateEnum.QuanBuTuiHuo.getValue()) {
					...
					return VipShopFlowEnum.FenZhanZhiLiu.getVipshop_state() + "";
				} else if (delivery_state == DeliveryStateEnum.HuoWuDiuShi.getValue()) {
					return VipShopFlowEnum.HuoWuDiuShi.getVipshop_state() + "";
				}
			}

...
/*	//验证数据
	public static boolean VerifyData(String verifyData, String clientid, String CLIENTFLAG, String CLIENTKEY, String CLIENTCONST, String CLIENTIP)
	{
	    if (verifyData.length() <= 5) return false;
	    String rand1 = verifyData.substring(0,4);
	    String rand2 = verifyData.substring(verifyData.length() - 4, verifyData.length()); 
	    String str = rand2 + CLIENTFLAG + CLIENTKEY + CLIENTCONST + rand1;
		String strMd5="";
		try {
//			strMd5 = MD5Util.md5(new String(str.getBytes("UTF8")));
			strMd5 = getMD5(str, "UTF8");
		} catch (Exception e) {
			e.printStackTrace();
		}
	    String CreateVerify = rand1 + strMd5.substring(7, 21) + rand2; 
	    return CreateVerify .equals(verifyData);
	 }

...
public class TransFlowFromJMSService {
		...
		try {
			...
			this.camelContext.addRoutes(new RouteBuilder() {
				...
				public void configure() throws Exception {
					...
					this.from(MQ_FROM_URI + "?concurrentConsumers=15").to("bean:transFlowFromJMSService?method=saveTransCwbFlow").routeId("transcwbdetail_");
				}
			});
	...
	public void saveTransCwbFlow(@Header("transCwbOrderFlow") String parm, @Header("MessageHeaderUUID") String messageHeaderUUID) {
		...
		try {
			...
			// 把未完成MQ插入到数据库中, start
			//消费MQ异常表
			this.mqExceptionDAO.save(MqExceptionBuilder.getInstance().buildExceptionCode(this.getClass().getSimpleName() + ".saveTransCwbFlow")

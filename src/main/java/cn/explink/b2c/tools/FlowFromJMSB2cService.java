...
public class FlowFromJMSB2cService {
	...
	public void init() {
		try {
			...
			//快递单类型的
			this.flowList.add(FlowOrderTypeEnum.YunDanLuRu.getMethod());//运单录入
			this.flowList.add(FlowOrderTypeEnum.LanJianRuZhan.getMethod());//揽件入站
			this.flowList.add(FlowOrderTypeEnum.LanJianChuZhan.getMethod());//揽件出站
			this.flowList.add(FlowOrderTypeEnum.LanJianQueRen.getMethod());//揽件确认

	...
	//获取配置信息
		...
		public String getFlowEnum(long flowordertype,long deliverystate) {
			...
			if(flowordertype==FlowOrderTypeEnum.YiShenHe.getValue()&&
					(deliverystate==DeliveryStateEnum.PeiSongChengGong.getValue()||deliverystate==DeliveryStateEnum.ShangMenTuiChengGong.getValue()||deliverystate==DeliveryStateEnum.ShangMenHuanChengGong.getValue() )){
		...
		private void sendCwbStatus_To_wx(Homegobj hg,long flowordertype){
			...
			try {
				...
				int i=0;
				while(true){
//					List<B2CData> datalist=b2cDataDAO.getDataListByFlowStatus(hg.getCustomerid(),hg.getMaxCount());
		...
		private void DealWithBuildXMLAndSending(Homegobj wx, List<B2CData> datalist)throws Exception {
			...
			String signXML = buildRequestOrdersString(wx, datalist, null,request_time);
			
			//logger.info("家有购物加密字符串signXML={}",signXML);
			
			String signed=MD5Util.md5(signXML+wx.getPrivate_key());
				
			String requestXML = buildRequestOrdersString(wx, datalist, signed,request_time);
			...
			if("00".equals(orderStatusResponse.getResponseHeader().getResp_code())){
				...
				for(B2CData b2cdata:datalist){
					if(orderStatusResponse.getResponseOrderBody()!=null){
						List<ResponseOrder> orderlist =	orderStatusResponse.getResponseOrderBody().getOrders().getOrderDto();
						if(orderlist!=null&&orderlist.size()>0){
							for(ResponseOrder order:orderlist){
							 int send_b2c_flag = "00".equals(order.getErr_code())?1:2;
							 String remark = order.getErr_desc();
							 
							 b2cDataDAO.updateB2cIdSQLResponseStatus(b2cdata.getB2cid(), send_b2c_flag, orderStatusResponse.getResponseHeader().getResp_msg()+remark);
							}
						}
					}
					...
					b2cDataDAO.updateB2cIdSQLResponseStatus(b2cdata.getB2cid(), send_b2c_flag, orderStatusResponse.getResponseHeader().getResp_msg());
				}
			}else{
				for(B2CData b2cdata:datalist){
					int send_b2c_flag=2;
					String err_msg=orderStatusResponse.getResponseHeader().getResp_msg();
					b2cDataDAO.updateB2cIdSQLResponseStatus(b2cdata.getB2cid(), send_b2c_flag, err_msg);
				}
			}
			//报文头显示失败
		...
		private void DealWithBuildXMLAndSending_duanxin(Homegobj wx, List<B2CData> datalist)throws Exception {
			...
			String signXML = buildDuanXinYuYueXML(wx, datalist, null,request_time);
			
			
			String signed=MD5Util.md5(signXML+wx.getPrivate_key());
				
			String requestXML = buildDuanXinYuYueXML(wx, datalist, signed,request_time);
			...
			if("00".equals(orderStatusResponse.getResponseHeader().getResp_code())){
				...
				for(B2CData b2cdata:datalist){
					int send_b2c_flag=1;
					b2cDataDAO.updateFlagAndRemarkByCwb(b2cdata.getB2cid(), send_b2c_flag,orderStatusResponse.getResponseHeader().getResp_msg());
				}
			}else{
				for(B2CData b2cdata:datalist){
					int send_b2c_flag=2;
					...
					b2cDataDAO.updateFlagAndRemarkByCwb(b2cdata.getB2cid(), send_b2c_flag,err_msg);
				}
			}
			//报文头显示失败
		...
		private String  buildRequestOrdersString(Homegobj wx,List<B2CData> datalist,String singed,String request_time) throws JsonParseException,JsonMappingException, IOException {
			String xml="";
				...
				if(singed!=null){
					...
					xml+="<Jiayou>"
						...
						+"<version>1.0</version>"
						+"<app_id>"+wx.getExpress_id()+"</app_id>"
						+"<charset>01</charset>"
					...
					if(singed!=null){
						...
						+"<orders>";
							...
							for(B2CData data:datalist){
								...
								xml+="<order>"
										+"<invc_id>"+note.getInvc_id()+"</invc_id>"
										+"<ord_id>"+note.getOrd_id()+"</ord_id>"
										+"<ord_stat>"+note.getOrd_stat()+"</ord_stat>"
										+"<stat_time>"+note.getStat_time()+"</stat_time>"
		...
		private String  buildDuanXinYuYueXML(Homegobj wx,List<B2CData> datalist,String singed,String request_time) throws JsonParseException,JsonMappingException, IOException {
			String xml="";
				...
				if(singed!=null){
					...
					xml+="<Jiayou>"
						+"<header>"
						+"<function_id>JIAYOU0004</function_id>"
						+"<version>1.0</version>"
						+"<app_id>"+wx.getExpress_id()+"</app_id>"
						+"<charset>01</charset>"
						+"<sign_type>02</sign_type>";
					...
					for(B2CData data:datalist){
						...
						xml+="<order>"
								+"<invc_id>"+note.getInvc_id()+"</invc_id>"
								+"<ord_id>"+note.getOrd_id()+"</ord_id>"
								+"<delivery_phone>"+note.getDelivery_phone()+"</delivery_phone>"
								+"<delivery_name>"+note.getDelivery_name()+"</delivery_name>"
								+"<delivery_time>"+note.getDelivery_time()+"</delivery_time>"
								+"<delivery_id>"+note.getDelivery_id()+"</delivery_id>"
								+"<remark_1>"+note.getRemark()+"</remark_1>"
								+"<remark_2></remark_2>"
							+"</order>";
					}
							xml+="</orders>"

...
public enum GuangZhouFlowEnum {
	...
	SC01("SC01", FlowOrderTypeEnum.RuKu.getValue(), "已接收（已入库）", 0), SC06_1("SC06", FlowOrderTypeEnum.ChuKuSaoMiao.getValue(), "库房出库扫描，所有的中转扫描", 0), SC06_2("SC06",
			FlowOrderTypeEnum.ZhongZhuanChuKuSaoMiao.getValue(), "中转中，所有的中转扫描", 0), SC06_3("SC06", FlowOrderTypeEnum.TuiHuoChuKuSaoMiao.getValue(), "分站退货出站", 0), SC02("SC02",
			FlowOrderTypeEnum.FenZhanLingHuo.getValue(), "配送中，所有分配配送员的扫描", 0),
	...
	private GuangZhouFlowEnum(String abc_state, int flowordertype, String text, int flag) {

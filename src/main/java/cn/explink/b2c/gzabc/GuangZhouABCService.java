...
import cn.explink.b2c.hzabc.HangZhouFlowEnum;
...
public class GuangZhouABCService {
	...
	public long feedback_status(int key) {
		...
		GuangZhouABC gzabc = getGuangZhouABCSettingMethod(key);
		if (!b2ctools.isB2cOpen(key)) {
			logger.info("未开广州ABC的对接!key={}", key);
			return -1;
		}
		logger.info("========广州ABC状态反馈任务调度开启==========");
		...
		calcCount += sendCwbStatusToABC(gzabc, FlowOrderTypeEnum.TuiHuoChuKuSaoMiao.getValue()); // 库房出库
	 ...
	 *            每次最大查询不超过50条
	 */
	private long sendCwbStatusToABC(GuangZhouABC gzabc, long flowordertype) {
		long calcCount = 0;
		try {
			...
			while (true) {
				...
				i++;
				if (i > 100) {
					String warning = "查询0广州ABC0状态反馈已经超过100次循环，每次50条，可能存在程序未知异常,请及时查询并处理!";
					logger.warn(warning);
					return 0;
				}

				if (yeMaiJiuDataList == null || yeMaiJiuDataList.size() == 0) {
					logger.info("当前没有要推送0广州ABC0的数据");
					return 0;
				}
			...
			String errorinfo = "发送0广州ABC0状态反馈遇到不可预知的异常" + e.getMessage();
	 ...
	 * @throws Exception
	 */
	private void DealWithBuildXMLAndSending(GuangZhouABC gzabc, List<B2CData> abcdataList) throws Exception {

		for (B2CData data : abcdataList) {
			long b2cid = data.getB2cid();
			String jsoncontent = data.getJsoncontent();
			GuangZhouXMLNote note = getGuangZhouABCXMLNoteMethod(jsoncontent);
			StringBuffer sub = buildSendABCXML(gzabc, note);

			String response_xml = postHTTPData_toGuangZhouABC(sub.toString(), gzabc.getFeedback_url(), gzabc.getLogisticProviderID(), gzabc.getPrivate_key());
			logger.info("状态反馈0广州ABC0[返回信息]-XML={}", response_xml);
	...
	private void dealWithFeedBackResponse(long b2cid, String response_xml) throws Exception {
		...
		if (response_xml.contains(",不存在的供货商")) {
			...
			if (respMap == null || respMap.size() == 0) {
				logger.warn("请求0广州ABC0解析xml为空，跳出循环,throw Exception,xml={}", response_xml);
				throw new RuntimeException("请求0广州ABC0解析xml为空，跳出循环,throw ");
			}
	...
	private StringBuffer buildSendABCXML(GuangZhouABC gzabc, GuangZhouXMLNote note) {
	...
	// 获取配置信息
	public GuangZhouABC getGuangZhouABCSettingMethod(int key) {
		GuangZhouABC gzabc = null;
		...
		if (objectMethod != null) {
			...
			gzabc = (GuangZhouABC) JSONObject.toBean(jsonObj, GuangZhouABC.class);
		} else {
			gzabc = new GuangZhouABC();
		}
		return gzabc;
	}

	// 获取 XML Note
	public GuangZhouXMLNote getTmallXMLNoteMethod(String jsoncontent) {
		try {
			JSONObject jsonObj = JSONObject.fromObject(jsoncontent);
			return (GuangZhouXMLNote) JSONObject.toBean(jsonObj, GuangZhouXMLNote.class);
		} catch (Exception e) {
			logger.error("获取GuangZhouXMLNote异常！jsoncontent:" + jsoncontent + e);
			return null;
		}
	}

	public String getGuangZhouABCFlowEnum(long flowordertype, long deliverystate) {
		if (flowordertype != FlowOrderTypeEnum.YiShenHe.getValue()) {
			for (GuangZhouFlowEnum TEnum : GuangZhouFlowEnum.values()) {
	...
	public static void main(String[] args) {
		...
		String str = new GuangZhouABCService().getGuangZhouABCFlowEnum(flowordertype, deliverystate);
		System.out.println(str);

	}

	public GuangZhouXMLNote getGuangZhouABCXMLNoteMethod(String jsoncontent) throws JsonParseException, JsonMappingException, IOException {
		return JacksonMapper.getInstance().readValue(jsoncontent, GuangZhouXMLNote.class);
	}

	public String postHTTPData_toGuangZhouABC(String xml, String url, String expressCode, String private_key) throws Exception {
		...
		logger.info("推送0广州ABC0,sShippedCode={},logicdata={},checkdata=" + checkdata, expressCode, xml);
